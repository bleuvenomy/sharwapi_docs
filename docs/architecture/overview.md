...
SharwAPI 采用 **宿主 + 插件** 架构。主程序本身是一个轻量的容器，它不包含具体的业务功能，而是仅负责为 **插件** 提供运行环境和基础 设施。
...
* **职责**:
    ...
    * **配置加载器**：扫描 `config/` 目录，为每个插件加载其专属的配置文件。
...
3.  **注册服务 (RegisterServices)**: 遍历所有插件，将插件定义的依赖服务注册到全局容器中。
